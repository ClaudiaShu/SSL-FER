...
def plot_spectrogram(spec, title=None, ylabel="freq_bin", aspect="auto", xmax=None):
    ...
    plt.savefig('/mnt/d/Data/Yuxuan/cache/test_mel.jpg')

def pad_audio(data, N_tar):
    ...
    if shape[0]>1:
        N_pad = N_tar - shape[0]
        audio = np.hstack((np.zeros([N_pad,]), data))
    else:
...
class videoCLR(Dataset):
    def __init__(self, *args, **kwargs):
        super(videoCLR, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        if self.trainMode == 'simclr':
            if self.dataMode == 1:
                # Get the image pairs
                Sample = {
                    'Anchor': self.transforms_img1(Image.open(data_info["Anchor"]).convert('RGB')),
                    'Positive': self.transforms_img2(Image.open(data_info["Positive"]).convert('RGB'))
                }
                return Sample

            elif self.dataMode == 2:
                # Get the audio pairs
                AudioPath = data_info["AudioPath"]
                Sample = {
                    'AuAnchor': self.transforms_aud1(self.load_audio(AudioPath, data_info["AuAnchor"])),
                    'AuPositive': self.transforms_aud2(self.load_audio(AudioPath, data_info["AuPositive"]))
                }
                return Sample
            ...
            if self.dataMode == 1:
                ...
                Sample = {
                    'Anchor': self.transforms_img1(Image.open(data_info["Anchor"]).convert('RGB')),
                    'Positive': self.transforms_img2(Image.open(data_info["Positive"]).convert('RGB')),
                    'NAnchor': self.transforms_img1(Image.open(data_info["NegativeAnchor"]).convert('RGB')),
                    'NPositive': self.transforms_img2(Image.open(data_info["NegativePositive"]).convert('RGB'))
                }
                return  Sample
                ...
                Sample = {
                    'AuAnchor': self.transforms_aud1(self.load_audio(AudioPath, data_info["AuAnchor"])),
                    'AuPositive': self.transforms_aud2(self.load_audio(AudioPath, data_info["AuPositive"])),
                    'AuNAnchor': self.transforms_aud1(self.load_audio(AudioPath, data_info["AuNegativeAnchor"])),
                    'AuNPositive': self.transforms_aud2(self.load_audio(AudioPath, data_info["AuNegativePositive"]))
                }
                return Sample
    ...
    def get_idx_mapping(self):
        ...
        for i in (pbar:= tqdm(range(len(self.paths)))):
            ...
            if self.dataset == 'Aff2':
                ...
                # audio_name = path.split('/') # todo: confirm this
                audio_name = '/'.join(path.split('/')[1:])
                audio_path = f"{self.prefix_vox1}/{audio_name}.wav"
            ...
            a_idx_p = 0
            if self.dataMode != 1:
                idx_ori = self.get_real_id(img_path[idx])
                idx_p_ori = self.get_real_id(img_path[idx_p])
                a_idx = self.get_audio_id(idx_ori, frames)
                a_idx_p = self.get_audio_id(idx_p_ori, frames)
                if a_idx is None or a_idx_p is None:
                    continue
            ...
            if self.trainMode == 'videoclr':
                ...
                a_idx_n2 = 0
                if self.dataMode != 1:
                    idx_n1_ori = self.get_real_id(img_path[idx_n1])
                    idx_n2_ori = self.get_real_id(img_path[idx_n2])
                    a_idx_n1 = self.get_audio_id(idx_n1_ori, frames)
                    a_idx_n2 = self.get_audio_id(idx_n2_ori, frames)
                    if a_idx_n1 is None or a_idx_n2 is None:
                        continue
            ...
            mapping[cpt] = {
                ...
                "AuAnchor": a_idx,
                "AuPositive": a_idx_p,
                "AuNegativeAnchor": a_idx_n1,
                "AuNegativePositive": a_idx_n2
            }
            cpt += 1
    ...
    def get_audio_id(self, ID, frames):
        ...
        # hf_len = int(np.floor(self.fps * length / 2))  # half of the number of clips in
        # if ID - hf_len >= 0 and ID - hf_len < frames - 1:
        #     return int(ID - hf_len)
        # elif ID - hf_len < 0:
        #     return 0
        # else:
        #     return ID - hf_len
